...
import {DATE_TOKENS} from './helpers/constants'
...
const tokenFunctions = {
  ...
  MMMM: (d, l) => l.months[d.getUTCMonth()],
  MMM: (d, l) => l.monthsShort[d.getUTCMonth()],
  ...
  D: d => d.getUTCDate(),
  dddd: (d, l) => l.weekdays[d.getUTCDay()],
  ddd: (d, l) => l.weekdaysShort[d.getUTCDay()],
  dd: (d, l) => l.weekdaysMin[d.getUTCDay()],
  d: d => d.getUTCDay(),
...
const swapTokenWithValue = curry((locale, date, token) => (
  tokenFunctions[token] ? tokenFunctions[token](date, locale) : token)
)

export default curry((locale, format, date) => {
  if (isValid(date) === false) return 'Invalid Date'

  return format.match(DATE_TOKENS)
    .map(swapTokenWithValue(locale, date))
    .join('')
})
