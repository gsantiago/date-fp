...
import PT_BR_LOCALE from '../locales/pt-br'

const format = formatWithLocale(EN_LOCALE)
...
describe('format', () => {
  ...
  it('should be curried', () => {
    ...
    assert.equal(formatWithLocale(EN_LOCALE)(str)(date), formatWithLocale(EN_LOCALE, str, date))
  })
  ...
  it('should support custom locales', () => {
    const formatPtBr = formatWithLocale(PT_BR_LOCALE)

    for (let i = 0; i < 12; i++) {
      const date = of([2017, i])
      assert.equal(formatPtBr('MMMM', date), PT_BR_LOCALE.months[i])
      assert.equal(formatPtBr('MMM', date), PT_BR_LOCALE.monthsShort[i])
    }

    for (let i = 0; i < 7; i++) {
      const date = add('days', i, of([2017, 0, 1]))
      assert.equal(formatPtBr('dddd', date), PT_BR_LOCALE.weekdays[i])
      assert.equal(formatPtBr('ddd', date), PT_BR_LOCALE.weekdaysShort[i])
      assert.equal(formatPtBr('dd', date), PT_BR_LOCALE.weekdaysMin[i])
    }
  })
})
